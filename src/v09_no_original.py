"""v09: v04 pipeline WITHOUT original data — test if 7K rows hurt."""
...
VERSION = "v09_no_original"
SEED = 42
N_FOLDS = 5

print('Loading (NO original data)...', file=sys.stderr, flush=True)
train = pd.read_csv('data-kaggle/train.csv')
test = pd.read_csv('data-kaggle/test.csv')
sub = pd.read_csv('data-kaggle/sample_submission.csv')

# NO ORIGINAL DATA MERGE
...
print(f'Train: {train.shape} (pure synthetic)', file=sys.stderr, flush=True)
...
# === TRAIN (v04 architecture, optimized weights) ===
...
json.dump({'cv_opt':round(best_a,6),'weights':list(best_w),'feats':train.shape[1],'no_original':True},
